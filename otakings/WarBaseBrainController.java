...
public abstract class WarBaseBrainController extends WarBaseBrain {
	...
	static WTask idle = new WTask(){
		String exec(WarBrain bc){
			...
			me.setDebugString(" sac : "+me.getNbElementsInBag());

			ArrayList<WarAgentPercept> AlliePercepts = (ArrayList<WarAgentPercept>) me.getPerceptsAllies();
			for (int i=0;i<AlliePercepts.size();i++) {
				if (AlliePercepts.get(i).getHealth()/AlliePercepts.get(i).getMaxHealth() < 0.4 ) {
					if (!me.isBagEmpty()) {
						me.setIdNextAgentToGive(AlliePercepts.get(i).getID());
			...
			if (me.getMaxHealth()*0.9 <= me.getHealth()) {
